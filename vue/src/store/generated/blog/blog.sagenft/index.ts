...
export default {
	...
	gas: "200000" }, memo})
		...
		async sendMsgDistribute({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgDistribute(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgDistribute:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgDistribute:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		...
		async MsgDistribute({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgDistribute(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgDistribute:Create Could not create message: ' + e.message)
				}
			}
		},

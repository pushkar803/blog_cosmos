...
export class MsgClientImpl implements Msg {
  ...
  Transfer(request: MsgTransfer): Promise<MsgTransferResponse> {
    ...
    const promise = this.rpc.request("blog.nft.Msg", "Transfer", data);
    return promise.then((data) => MsgTransferResponse.decode(new Reader(data)));
  }

  Burn(request: MsgBurn): Promise<MsgBurnResponse> {
    const data = MsgBurn.encode(request).finish();
    const promise = this.rpc.request("blog.nft.Msg", "Burn", data);
    return promise.then((data) => MsgBurnResponse.decode(new Reader(data)));
  }

  Mint(request: MsgMint): Promise<MsgMintResponse> {
    const data = MsgMint.encode(request).finish();
    const promise = this.rpc.request("blog.nft.Msg", "Mint", data);
